...
/*UPDATE: as of version 1.3, you may omit the date if you would like. But if you
  ...
  let postsArray = [
    ...
    postsArray.sort((a, b) => {
        const dateA = a[0].match(/\d{4}-\d{2}-\d{2}/)?.[0] || "";
        const dateB = b[0].match(/\d{4}-\d{2}-\d{2}/)?.[0] || "";
        return dateB.localeCompare(dateA);
    });
    
    function assignPostIDs(array) {
        for (let i = 0; i < array.length; i++) {
          const post = array[i];
          const id = array.length - i;
          if (post.length === 1) {
            post.push(""); // titulo vazio se nao tive
          }
          post[2] = id;
        }
      }
    assignPostIDs(postsArray)
    
    async function formatPostLink(i) {
        let postTitle_i = postsArray[i][1]; //titulo alternativo
        if (!postTitle_i) { //se nao tiver
          if (postDateFormat.test(postsArray[i][0].slice(6, 17))) {
        ...
        let href = relativePath + '/' + postsArray[i][0];
        let tittle = `${postTitle_i}: Progress Report #${postsArray[i][2]}`
        let date = ""
        let desc = ""
        let imgSrc = `/media/posts/thumb-${postsArray[i][2]}.png`

        if (postDateFormat.test(postsArray[i][0].slice(6, 17))) {
            date = `${postsArray[i][0].slice(6, 16)}`
        } else
          date = "undefinied";
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Post not found: ${href}`);
            }
    
            const postHtml = await response.text();
    
            // temporario, apena spara armazenar
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = postHtml;
            
            const postText = tempDiv.querySelector('span#resume') //span#resume = id /// span.resume = class
                ? tempDiv.querySelector('span#resume').textContent 
    ...
    for ( let i = 0; i < postsArray.length; i++ ) {
      postListHTML += await formatPostLink(i);
    }
    postListHTML += "</ul>";
    ...
    for ( let i = 0; i < numberOfRecentPosts; i++ ) {
      recentPostListHTML += await formatPostLink(i);
    }
    ...
    if (document.getElementById("recentPosts")) {
      document.getElementById("recentPosts").innerHTML = recentPostListHTML;
    }
